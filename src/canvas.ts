...
// ペン色は setColor() で切り替えられ、各ストロークは書いた時点の色を保持する
// （墨で書いた線を朱に浸け替えても、既存の線は墨のまま残る）。
...
/** 1 本の線とその色。ペンを浸け替えても既存の線の色は変わらない。 */
interface Mark {
  pts: Stroke;
  color: string;
}

const INK_COLOR = "#26355c"; // 既定の墨（紺）
...
export class InkCanvas {
  ...
  private currentColor = INK_COLOR;
  private activePointerId: number | null = null;
  private dpr = 1;
  private lastP = 0.5;
  /** いま浸けているペンの色。次に書く線に使われる。 */
  color = INK_COLOR;
  ...
  /** 判定対象になる線の本数（main.ts のキャプチャ整合性チェック用）。 */
  ...
  private down(e: PointerEvent) {
    ...
    this.currentColor = this.color; // この線が使う色を接地時に確定する
    try {
  ...
  private finish(e: PointerEvent, addFinalPoint: boolean) {
    ...
    if (this.current.length > 1) {
      ...
      this.marks.push({ pts: this.current, color: this.currentColor });
  ...
  private drawLiveDot(pt: Point) {
    ...
    this.drawDot(ctx, pt, this.currentColor, 1);
    ctx.restore();
  }
  ...
  private drawStrokeCurve(
    ...
    const n = s.length;
    if (n === 1) {
      this.drawDot(ctx, s[0], color, alpha, blur);
      return;
    }
    if (n < 2) return;
    ctx.save();
  ...
  redraw() {
    ...
    ctx.setTransform(this.dpr, 0, 0, this.dpr, 0, 0);
    for (const m of this.marks) this.drawStrokeCurve(ctx, m.pts, m.color, 1);
    if (this.current && this.current.length > 1) {
      this.drawStrokeCurve(ctx, this.current, this.currentColor, 1);
    }
    ctx.setTransform(1, 0, 0, 1, 0, 0);
  }
  ...
  /** いま描かれている紙面をそのまま PNG データURLに（ページめくりの葉に貼る用）。 */
  snapshot(): string {
  ...
  fadeOut(): Promise<void> {
    ...
    const perStrokeDelay = Math.min(120, 1400 / Math.max(marks.length, 1));
    const fadeDur = 1100;
    const total = perStrokeDelay * marks.length + fadeDur;
    ...
    return new Promise((resolve) => {
      const frame = (now: number) => {
        ...
        for (let i = 0; i < marks.length; i++) {
          ...
          if (alpha > 0.01) {
            anyVisible = true;
            this.drawStrokeCurve(ctx, marks[i].pts, marks[i].color, alpha, (1 - alpha) * 3.5);
          }
        }
        ctx.setTransform(1, 0, 0, 1, 0, 0);
